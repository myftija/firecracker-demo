...
DATA_DIR="output"
DEST="$PWD/restore_to_network_ready_tcp22_probe.log"
...
do
  ...
  restore_to_network_ready_ms=$(cat "$log_file" | grep "RESTORE_TO_NETWORK_READY_TCP22_MS" | head -1 | awk '{print $2}')
  ...
  echo "$i restore_to_network_ready_tcp22 $restore_to_network_ready_ms ms" >> "$DEST"
done
